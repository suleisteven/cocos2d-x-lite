...
//				shareAction.share();
	...
	public static boolean isAuthorized(int platform) {
		...
		if (target != null && target != SHARE_MEDIA.GENERIC) {
			boolean isAu = mShareAPI.isAuthorize(mActivity,getPlatform(platform));
			Log.d(TAG, "xxxxxx isAuthorized="+ isAu);
			return  isAu;
		}
	...
	private static UMAuthListener mAuthListener = new UMAuthListener() {
		...
		private String[] getAuthMap(Map<String, String> data) {
			if (data != null
				...
				// fix bug，微信这里只会返回openid，不会返回uid；其他平台uid和openid都会返回，且是一样的值
				else if(data.containsKey("openid"))
				{
					authData[1] = data.get("openid");
				}else
				{
					authData[1] = "";
				}
			...
			runNativeCallback(new Runnable() {
				...
				public void run() {
					OnShareComplete(getPlatformInt(share_media), StatusCode.ST_CODE_ERROR,
							share_media.toString()+(null != throwable ? throwable.getMessage() : ""));
				}
